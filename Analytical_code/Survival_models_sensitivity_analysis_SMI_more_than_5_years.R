...
load(file = "path/Comorbidity_SMI/Data/data_senstivity_analysis_SMI_more_than_5_years.RData")

#Testing the proportionality assumption using Schoenfeld residuals

map(.x = data_senstivity_analysis_SMI_more_than_5_years,
...
mfull <- imap_dfr(data_senstivity_analysis_SMI_more_than_5_years,
  ...
  transmute(cohort,
            `Severe mental illness >5 years before the physical health condition` = paste(formatC(round(estimate, 2), format = "f", digits = 2),
                                                                                   paste0("(", 
                                                                                          ...
                                                                                          ")"))) %>%
  write.csv(file = "path/Comorbidity_SMI/Results/HR_sens_analysis_SMI_more_than_5_years.csv",
            row.names = FALSE)
