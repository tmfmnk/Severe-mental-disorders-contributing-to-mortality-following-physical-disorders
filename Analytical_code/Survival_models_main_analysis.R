...
imap(data_main_analysis,
     function(data, cohort_names) {
       ...
       p <- ggsurvplot(surv_fit(Surv(years_until_death_or_censoring, mortality) ~ exposure, 
                       ...
                       xlim = c(0, 20),
                       break.x.by = 5,
                       legend.labs = c("People without severe mental disorder", "People with severe mental disorder"),
                       title = paste0("Survival following ", ifelse(cohort_names == "Parkinson's disease", cohort_names, tolower(cohort_names)), " by severe mental disorder status"),
