...
cohorts <- map(.x = cohorts,
               ~ .x %>%
                 left_join(smi_history,
                           by = "RODCIS2") %>%
                 mutate(exposure = case_when(DATPRI > DATUKO_smi ~ "exposed",
...
data_main_analysis <- map(.x = cohorts_baseline,
                          ~ .x %>%
...
imap_dfr(.x = data_main_analysis,
  ...
  arrange(order) %>%
  select(-order) %>%
  write.csv(file = "path/Comorbidity_SMI/Results/Descriptives_main_analysis.csv",
            row.names = FALSE)
             
#Export data

save(data_main_analysis, 
     file = "path/Comorbidity_SMI/Data/data_main_analysis.RData")
