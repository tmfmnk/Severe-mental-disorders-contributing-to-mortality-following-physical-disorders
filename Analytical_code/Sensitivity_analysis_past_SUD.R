...
past_sud <- map(.x = data_main_analysis,
                ~ .x %>%
                  ...
                  mutate(past_sud = str_detect(ZDG_historic, "^F1") * past_hosp) %>%
                  summarise(past_sud = sum(past_sud) != 0) %>%
                  ungroup())
...
mfull <- imap_dfr(data_models,
                  ~ tidy(coxph(Surv(years_until_death_or_censoring, mortality) ~ exposure + VEK + POHL + discharge_year + as.factor(past_sud) + strata(strata), 
                               data = .x),
                         conf.int = TRUE,
                         exponentiate = TRUE) %>%
                    mutate(cohort = .y) %>%
...
mfull %>%
  arrange(cohort) %>%
  transmute(cohort,
            `Past SUD` = paste(formatC(round(estimate, 2), format = "f", digits = 2),
                               paste0("(", 
                                      formatC(round(conf.low, 2), format = "f", digits = 2),
                                      "; ",
                                      formatC(round(conf.high, 2), format = "f", digits = 2),
                                      ")"))) %>%
  write.csv(file = "path/Comorbidity_SMI/Results/HR_sens_analysis_past_SUD.csv",
            row.names = FALSE)
