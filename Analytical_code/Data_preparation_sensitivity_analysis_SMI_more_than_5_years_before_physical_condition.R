...
cohorts <- map(.x = cohorts,
               ~ .x %>%
                 mutate(smi_more_tnan_5_years_start = ymd("19990101"),
                        smi_more_than_5_years_end = (ymd(DATPRI) %m-% years(5)) - 1) %>%
                 left_join(smi_history,
                           by = "RODCIS2") %>%
                 mutate(exposure = case_when(int_overlaps(interval(smi_more_tnan_5_years_start, smi_more_than_5_years_end), interval(ymd(DATUKO_smi), ymd(DATUKO_smi))) ~ "exposed",
...
data_senstivity_analysis_SMI_more_than_5_years <- map(.x = cohorts_baseline,
                                                      ~ .x %>%
...
imap_dfr(.x = data_senstivity_analysis_SMI_more_than_5_years,
         ~ .x %>%
           group_by(exposure) %>%
           summarise(cohort = .y,
  ...
  arrange(order) %>%
  select(-order) %>%
  write.csv(file = "path/Comorbidity_SMI/Results/Descriptives_senstivity_analysis_SMI_more_than_5_years.csv",
            row.names = FALSE)

#Export data

save(data_senstivity_analysis_SMI_more_than_5_years, 
     file = "path/Comorbidity_SMI/Data/data_senstivity_analysis_SMI_more_than_5_years.RData")
