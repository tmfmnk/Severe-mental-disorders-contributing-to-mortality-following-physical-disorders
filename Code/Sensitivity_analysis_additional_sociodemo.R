...
data_models <- map(.x = data_main_analysis,
                   ~ .x %>%
                     group_by(strata) %>%
                     filter(all(ZAM %in% c(0:9) & STAV %in% c(0:5))) %>%
                     ungroup())
...
mfull <- imap_dfr(data_models,
                  ~ tidy(coxph(Surv(years_until_death_or_censoring, mortality) ~ exposure + VEK + POHL + discharge_year + ZAM + STAV + strata(strata), 
                               data = .x),
                         conf.int = TRUE,
                         exponentiate = TRUE) %>%
                    mutate(cohort = .y)  %>%
  ...
  transmute(cohort,
            `Additional sociodemo` = paste(formatC(round(estimate, 2), format = "f", digits = 2),
                                           paste0("(", 
                                                  ...
                                                  ")"))) %>%
  write.csv(file = "path/HR_sens_analysis_additional_sociodemo.csv",
            row.names = FALSE)