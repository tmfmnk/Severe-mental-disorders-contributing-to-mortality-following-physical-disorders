...
save(lyl_smi, file = "path/lyl_smi.RData") 
save(lyl_no_smi, file = "path/lyl_no_smi.RData") 

#Load data

load(file = "path/lyl_smi.RData")
load(file = "path/lyl_no_smi.RData") 
...
ggsave(file = "path/Diff_life_years_lost_age_range.eps",
       device = "eps",
       width = 40,
       height = 20,
       units = "cm",
  ...
  transmute(cohort,
            lyl_diff_estimate_with_ci = paste(formatC(round(lyl_estimate.TotalLYL, 2), format = "f", digits = 2),
                                              paste0("(", 
                                                     ...
                                                     ")"))) %>%
  write.csv(file = "path/Diff_life_years_lost.csv",
            row.names = FALSE)
