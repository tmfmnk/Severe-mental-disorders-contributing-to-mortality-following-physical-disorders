  ...
  transmute(cohort,
            estimate = paste(formatC(round(estimate, 2), format = "f", digits = 2),
                             paste0("(", 
                                    ...
                                    ")"))) %>%
  write.csv(file = "path/HR_main_analysis.csv",
            row.names = FALSE)
...
ggsave(file = "path/HR_plot_main_analysis.eps",
       device = "eps",
       width = 40,
       height = 20,
       units = "cm",
...
imap(data_main_analysis,
     function(data, cohort_names) {
       ...
       p <- ggsurvplot(surv_fit(Surv(years_until_death_or_censoring, mortality) ~ exposure, 
                       ...
                       xlim = c(0, 20),
                       break.x.by = 5,
                       legend.labs = c("People without SMI", "People with SMI"),
                       title = paste0("Survival following ", ifelse(cohort_names == "Parkinson's disease", cohort_names, tolower(cohort_names)), " by SMI status"),
